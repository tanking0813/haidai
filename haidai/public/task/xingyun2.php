...
$category_list = $Xingyun->PublicRes('get_category_list',[]); //所有分类类别
 $cat_goods_all = array_reduce($Category->getGoodsAll(1), function ($result, $value) {
                            return array_merge($result, array_values($value));
                        }, array());
                $t2 = microtime(true);
echo '耗时'.round($t2-$t1,3).'秒<br>';
        print_r(1111,$cat_goods_all);die;
//查找商品分类
...
echo '耗时'.round($t2-$t1,3).'秒<br>';
        ...
        foreach ($cat_goods_all as $h => $j) {
            ...
            if(empty($XyAllGoods[$j['sku_id']])){
                ...
                $category_detail =  $Xingyun->PublicRes('get_goods_detail',$order);// 查询详情
                ...
                $install_goods['title'] = $j['sku_name'];  //标题
                $install_goods['content'] = json_encode($category_detail['data']['sku_detail']);  // 详情
                ...
                $install_goods['stock'] = $category_price['data'][0]['sku_stock_num_show'];  // 库存
                ...
                $install_goods['sale_type'] = $category_detail['data']['sku_putaway_status'];
                // $install_goods['sku_no'] = $j['sku_id'];  // 商品编码
                $install_goods['create_time'] = time();  
                $install_goods['delivery_city'] = $j['warehouse_name'];  // 发货地（仓库名称）

                $install_goods['country_id'] = empty($Country_list[$category_detail['data']['country_name']]['country_id']) ? $Country_list[$DomainCountry->Country_switch($category_detail['data']['country_name'])]['country_id']  :$Country_list[$category_detail['data']['country_name']]['country_id']  ;   // 国家id 
                $install_goods['xy_price'] =  json_encode( $category_price['data'][0]);  // 价格
